...
void onNewColorSample(ColorNode node, ColorNode::NewSampleReceivedData data)
{
	...
	CvMat *Ma=cvCreateMat(480,640,CV_8UC3);
	...
	for(int i=0;i<480;i++)
	{
		...
		{
			CvScalar temp;
			int index_0=3*(i*640+j);
			int index_1=3*(i*640+j)+1;
			int index_2=3*(i*640+j)+2;

			temp.val[0]=int(data.colorMap[index_0]);
			temp.val[1]=int(data.colorMap[index_1]);
			temp.val[2]=int(data.colorMap[index_2]);
			cvSet2D(Ma,i,j,temp);
	...
	string image_path="/home/nkallaku/data/img/color/color_"+index_str+".jpg";

	printf("came here\n");
	string output_string="color_"+index_str+".jpg";

	ros::NodeHandle nh;
	ros::Publisher pub = nh.advertise<sensor_msgs::Image>("camera/rgb/image_color", 1000);

	cv_bridge::CvImage out_msg;
	out_msg.encoding = sensor_msgs::image_encodings::BGR8; // Or whatever
	out_msg.image    = Ma_mat; // Your cv::Mat
	...
	cvSaveImage(image_path.c_str(),Ma);
	cvReleaseMat(&Ma);
